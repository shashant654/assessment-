...
db = db.getSiblingDB('zangoh');

// Create conversations collection and add sample data
...
// Create agents collection and add sample data
...
// Create knowledge bases collection
...
db.knowledgebases.insertMany(knowledgeBases);

// Create indexes for better query performance
db.conversations.createIndex({ "id": 1 }, { unique: true });
db.conversations.createIndex({ "status": 1 });
db.conversations.createIndex({ "alertLevel": 1 });
db.conversations.createIndex({ "agent.id": 1 });

db.agents.createIndex({ "id": 1 }, { unique: true });
db.knowledgebases.createIndex({ "id": 1 }, { unique: true });
